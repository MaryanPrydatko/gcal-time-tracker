...
(() => {
  ...
  // [from, to] window for "weeksBack full weeks + the current week".
  const range = (weeksBack, now = new Date()) => {
  ...
  // Sum matching occurrences into weekly buckets (oldest -> newest, the last
  // row is the current week). Case-insensitive substring match on the name.
  const weeklyHours = (occurrences, query, weeksBack, now = new Date()) => {
    const cur = weekStart(now);
